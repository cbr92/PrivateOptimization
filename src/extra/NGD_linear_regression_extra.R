...
NGD.Huber <- function(x,y,k=1.345,fisher_beta=0.7101645,scale=T,private=T,mu=1,maxiter=100,eps=1e-5,beta0=rep(0,dim(x)[2]),s0=1,stepsize=NULL,s.min=0.01,stopping=0,mnorm=sqrt(2),suppress.inference=FALSE)
{
  ...
  np_grad_traj=priv_grad_traj=beta2_traj=rep(NA,maxiter+1)
  sigma_vec<-s0

  beta2_traj[1]<-beta0[2] #this is used to generate Figure 1
...
# Location estimation with known scale
  ...
  while(iter<maxiter & stop_flag < 1){
    ...
    beta2_traj[iter+1]<-beta0[2] #this is used to generate Figure 1
  ...
  out$private=private
  out$noise=noise
  out$beta2_traj=beta2_traj #this is used to generate Figure 1
  if(suppress.inference==FALSE){

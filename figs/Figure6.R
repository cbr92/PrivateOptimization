...
for(j in 1:10){
  niter<-rep(NA,ns)
  conv<-rep(NA,ns)
  betamat<-matrix(nrow=ns,ncol=4)
  for(i in 1:ns){
    x=matrix(rnorm(n*4,mean=0,sd=2),nrow=n)
    x[,1]<-rep(1,n)
    y=as.vector(x%*%beta_gen+rnorm(n,mean=0,sd=s0))
    out=NoisyNewton(x=x,y=y,private=T,mu=2,scale=T,stepsize=stepsizes[j],maxiter=20,suppress.inference=TRUE,stopping="non-private",mnorm=sqrt(2)) #vary the step size
    betamat[i,]<-out$beta
    
    niter[i]<-out$iter
    conv[i]=out$nonpriv_converge
  }
  
  diverged[j]<-sum(niter<20 & conv==0)/ns

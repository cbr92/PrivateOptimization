...
covmat23=covmat23NC=covmat23NP=matrix(0,nrow=2,ncol=2)
center23=center23NP=rep(0,2)
betamat= betamatNP=matrix(NA,nrow=ns,ncol=p)
quadform23=quadform23NC=quadformNP=rep(NA,ns)
...
for(i in 1:ns){
  ...
  outNP=NGD.Huber(x=x,y=y,private=F,scale=T,maxiter=55,mnorm=sqrt(2),stopping=0,suppress.inference=FALSE)
  
  betamat[i,]<-out$beta
  betamatNP[i,]<-outNP$beta
  
  covmat23<-covmat23+(out$variances[2:3,2:3])/ns
  covmat23NC<-covmat23NC+(out$sandwich[2:3,2:3]/n)/ns
  covmat23NP<-covmat23NP +(outNP$variances[2:3,2:3])/ns
  
  center23<-center23+(out$beta[2:3])/ns
  center23NP<-center23NP+(outNP$beta[2:3])/ns
...
ellipse23<-ellipse(covmat23,centre=center23)
ellipse23NC<-ellipse(covmat23NC,centre=center23)
ellipse23NP<-ellipse(covmat23NP,centre=center23NP)

par(mfrow=c(1,1))
plot(betamat[,2],betamat[,3],xlim=c(0.67,1.38),ylim=c(0.68,1.22),xlab=expression(paste(beta)[2]),ylab=expression(paste(beta)[3]), main=expression(paste("1-GDP Estimates of (",beta[2],",",beta[3],")")),pch="*", col="cornflowerblue")
lines(ellipse23[,1],ellipse23[,2],col=1)
lines(ellipse23NC[,1],ellipse23NC[,2],col=1,lty=3)
points(x=1,y=1,col=3,pch=16)
points(betamatNP[,2],betamatNP[,3], col="gray", pch="*")
lines(ellipse23NP[,1], ellipse23NP[,2],col=4)
legend("bottomleft",legend=c("True parameter value","Avg 95% Conf Region, corrected","Avg 95% Conf Region, uncorrected"),lty=c(NULL,3,1),col=c(3,2,2),pch=c(16,NULL,NULL),bty='n',cex=0.8)

par(mfrow=c(1,2))
plot(betamat[,2],betamat[,3],xlim=c(0.61,1.35),ylim=c(0.7,1.25),xlab=expression(paste(beta)[2]),ylab=expression(paste(beta)[3]), main=expression(paste("1-GDP Estimates of (",beta[2],",",beta[3],")")),pch="*")
lines(ellipse23[,1],ellipse23[,2],col=2)
lines(ellipse23NC[,1],ellipse23NC[,2],col=2,lty=3)
points(x=1,y=1,col=3,pch=16)
legend("bottomleft",legend=c("True parameter value","Avg 95% Conf Region, corrected","Avg 95% Conf Region, uncorrected"),lty=c(NULL,3,1),col=c(3,2,2),pch=c(16,NULL,NULL),bty='n',cex=0.8)

plot(betamatNP[,2],betamatNP[,3],xlim=c(0.61,1.35),ylim=c(0.7,1.25),xlab=expression(paste(beta)[2]),ylab=expression(paste(beta)[3]), main=expression(paste("Nonprivate Estimates of (",beta[2],",",beta[3],")")),pch="*")
lines(ellipse23NP[,1],ellipse23NP[,2],col=2)
points(x=1,y=1,col=3,pch=16)
legend("bottomleft",legend=c("True parameter value","Avg 95% Conf Region"),lty=c(NULL,1),col=c(3,2),pch=c(16,NULL),bty='n',cex=0.8)

#save(betamat, betamatNP, covmat23, covmat23NC, covmat23NP, ellipse23, ellipse23NC, ellipse23NP, center23, center23NP, file="~/Documents/privacy/coverage_ellipse_updated.Rdata")
